...
function assertReadOnly(sql) {
    ...
    if (firstWord !== "SELECT" && firstWord !== "WITH") {
        throw new Error(`Read-only: only SELECT / WITH queries are allowed (got "${firstWord ?? "?"}").`);
    }
...
function friendlyError(err) {
    ...
    if (/permission|denied|Access Denied|forbidden|403/i.test(msg)) {
        return (`BigQuery denied this request. Your Google account needs read access to the ` +
    ...
    if (/could not load the default credentials|Application Default Credentials|reauth|invalid_grant/i.test(msg)) {
        return (`Not signed in to Google. Re-run the "Install Starface Data" app, or run ` +
...
const HOUSE_RULES = "Starface data conventions: start with `mart.*` tables (mart.orders is the canonical " +
...
server.registerTool("describe_table", {
    ...
    try {
        ...
        const lines = fields.map((f) => `  ${f.name}: ${f.type}${f.mode && f.mode !== "NULLABLE" ? ` (${f.mode})` : ""}`);
        const out = [
            `${dataset}.${table}`,
...
server.registerTool("query", {
    ...
    description: `Run a read-only (SELECT/WITH) BigQuery Standard SQL query against the Starface datalake and get rows back (capped at ${MAX_ROWS} rows). ` +
        HOUSE_RULES,
    inputSchema: {
